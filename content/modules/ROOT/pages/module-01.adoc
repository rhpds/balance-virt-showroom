...
NOTE: This feature is similar to VMware's Compute vMotion feature, and Distributed Resource Scheduler (DRS) for VM placement.
...
NOTE: To see the node the Virtual Machine is currently running on, look at the *General* box on the right hand side of the *VirtualMachine details* page for the *Node* name. In the example, the node is *worker-cluster-m2ssq-1*.
+
...
NOTE: Looking back at the *General* box on the right hand side of the *VirtualMachine details* page, you can see that the node has changed.
...
NOTE: The requirements for this type of migration are the same as the previous lesson.
...
NOTE: Looking back at the *General* box on the right hand side of the *VirtualMachine details* page, you can see that the node has changed, specifically to the one you selected from the list.
...
+
NOTE: It may take over a minute for the migration to start, and five or more minutes for the migration to complete in the lab environment.
