...
A decentralized live migration is an advanced variation of a storage live migration, which we covered earlier in this lab, that allows you to migrate a running *VirtualMachine* between *namespaces* and *clusters*.
...
NOTE: This portion of the lab introduces a new capability that just became GA in OpenShift 4.21. As such, the UI-based workflow is not fully developed and should improve dramatically in upcoming OpenShift releases. During this lab we will introduce you to the UI workflow, but a majority of the tasks will be performed through the CLI.
...
In this module we explored decentralized live migration within a cluster, across namespaces. This is the basic technology that allows us to live migrate workloads between remote clusters, and helps set us up for the future of a true hybrid multicloud environment.
